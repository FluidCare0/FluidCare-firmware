...
float iqrMedianWeight() {
  ...
  if (ic == 0) return rawToGrams(tmp[n / 2]);  // fallback: raw median
...
void sendSensorData() {
  ...
  if (isnan(weight)) return;  // not enough samples yet

  // Delta-based suppression
  if (fabsf(weight - lastTransmittedWeight) <= TRANSMIT_DELTA_G) return;
