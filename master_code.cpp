...
// ===============================================================
// Helper: Parse MAC string → byte array
// ===============================================================
...
// ===============================================================
// MQTT Callback (Receive messages from backend)
// ===============================================================
void mqttCallback(char *topic, byte *payload, unsigned int length)
{
  ...
  Serial.printf("📩 MQTT msg: req=%d, mac=%s, node_id=%s\n",
                reqCode, macStr, nodeId ? nodeId : "");

  // ------------------------------------------------------------------
  // 202 – Backend assigned a Node-ID → forward to node via ESP-NOW
  // ------------------------------------------------------------------
  ...
  // ------------------------------------------------------------------
  // 204 – Backend confirmed reset → forward ACK to node via ESP-NOW
  // ------------------------------------------------------------------
  else if (reqCode == RES_RESET_ACK)
  {
    ...
    if (!parseMacString(macStr, nodeMac))
    {
      Serial.printf("❌ Invalid MAC in reset ACK: %s\n", macStr);
      return;
    }

    if (!addPeerIfNeeded(nodeMac))
    {
      Serial.println("❌ Cannot add peer for reset ACK, skipping");
      return;
    }

    Serial.printf("🔄 Forwarding Reset ACK (204) → %02X:%02X:%02X:%02X:%02X:%02X\n",
                  nodeMac[0], nodeMac[1], nodeMac[2],
                  nodeMac[3], nodeMac[4], nodeMac[5]);
    ...
    for (int i = 0; i < 3; i++)
    {
      esp_err_t result = esp_now_send(nodeMac, (uint8_t *)&ack, sizeof(ack));
      if (result == ESP_OK)
      {
        Serial.printf("✅ Reset ACK sent to node (attempt %d)\n", i + 1);
        break;
      }
      else
      {
        Serial.printf("⚠️ Reset ACK send failed (attempt %d): %d\n", i + 1, result);
        delay(100);
      }
    }
  }
...
void connectMQTT()
{
  ...
  if (mqttClient.connect(MQTT_CLIENT_ID, MQTT_USER, MQTT_PASS))
  {
    ...
    // Subscribe to reset ACK topic (backend → master → node)
    mqttClient.subscribe("be_project/node/reset/ack");
    Serial.println("📬 Subscribed to: be_project/node/reset/ack");
  }
  else
  {
...
void onDataRecv(const uint8_t *mac, const uint8_t *incomingData, int len)
{
  ...
  // ------------------------------------------------------------------
  // 201 – Node-ID registration request
  // ------------------------------------------------------------------
  else if (data.request_code == REQ_NODE_ID)
  {
    Serial.printf("📨 Node-ID request from %s\n", macStr);

    // Ensure peer is tracked so we can send back later
    addPeerIfNeeded(mac);
    ...
    {
      Serial.printf("📤 Forwarded Node-ID request to server for MAC: %s\n", macStr);
    }
    else
    {
  ...
  // ------------------------------------------------------------------
  // 203 – Node Reset request → forward to backend
  // ------------------------------------------------------------------
  else if (data.request_code == REQ_RESET)
  {
    Serial.printf("🔄 Reset request (203) from %s\n", macStr);

    // Make sure we can send back to this node later
    addPeerIfNeeded(mac);
    ...
    {
      Serial.printf("📤 Forwarded Reset request to backend for MAC: %s\n", macStr);
    }
    else
    {
      Serial.println("❌ MQTT publish of reset request failed!");
    }
  }
}
